...
public class Square5 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Square5 frame = new Square5(null,null,null);
	 ...
	 */
	public Square5(DataInputStream dis, DataOutputStream dos, JSONObject user) throws JSONException {
		...
		btnUsername.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					switch (func) {
						case "Quit": {
							BUS.sendEndGame(dos, Square5.this.user.getInt("player ID")+"","Quit");
							break;
						}
						default: { //Edit profile
							// Edit information
							setVisible(false);
							prof.mainFrame(Square5.this);
							prof.user(Square5.this.user);
							prof.loadProfile();
		...
		listPlayer.setCellRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList list, Object value, int index,
                 ...
                 if (Square5.this.playerList == null || Square5.this.playerList.size() < 1) return c;
                 String nextUser = value.toString();
                 setText(nextUser);
                 if (Square5.this.onlineList == null || Square5.this.onlineList.size() < 1) return c;
                 if (Square5.this.onlineList.contains(nextUser)) setBackground(Color.GREEN);
		...
		listRanking.setCellRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList list, Object value, int index,
                 ...
                 if (Square5.this.rankingList == null || Square5.this.rankingList.size() < 1) return c;
                 String nextUser = value.toString();
                 setText(nextUser);
                 try {
					if (nextUser.equals(Square5.this.user.getString("Username"))) setBackground(Color.YELLOW);
			...
			button[i].addActionListener((ActionListener) new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						int findVal = Square5.this.match.getInt("NextNumber");
				...
				try {
					...
					switch (func) {
						case "Match": {
							...
							BUS.sendSimpleRequest(dos, "Join room",Square5.this.user.getString("player ID"));
							break;
						}
						case "Start Game": {
							BUS.sendSimpleRequest(dos, "StartGame",Square5.this.user.getString("player ID"));
							break;
						}
						default: {
	...
	private void initInfor(DataInputStream dis, DataOutputStream dos) {
		...
		Square5.this.game_status.start();
	}
                ...
                while (true) { 
                    try { 
                        ...
                        switch (obj.getString("Type")) {
	                        case "Player List": {
	                			Square5.this.playerList = json2Array(obj.getJSONArray("Player"));
	                			Square5.this.listPlayer.setListData(Square5.this.playerList.toArray());
	                			Square5.this.listPlayer.updateUI();
	                        	break;
	                        }
							case "Ranking List": {
								Square5.this.rankingList = json2Array(obj.getJSONArray("Player"));
								Square5.this.listRanking.setListData(Square5.this.rankingList.toArray());
								Square5.this.listRanking.updateUI();
								break;
							}
							case "Online List": {
								Square5.this.onlineList = json2Array(obj.getJSONArray("Player"));
								Square5.this.listPlayer.updateUI();
								break;
							}
							case "EditProfile": {
								if (obj.has("Errorcode") && obj.getString("Errorcode").equals("Er0")) { //Success
									...
									Square5.this.prof.user(obj);
									Square5.this.prof.loadProfile();
								}
								else {
	                        ...
	                        case "UpdateMatch": {
	                        	...
	                        	Square5.this.match.put("NextNumber", number);
	                        	UpdateMatch();
	                        }
	                        case "CreateRoom": {
	                        	textField_1.setText(Square5.this.user.getString("Username"));
	                        	break;
	                        }
	                        case "JoinRoom": {
	                        ...
	                        case "StartGame": {
	                        	Square5.this.match = obj;
	                        	Square5.this.t = new Timer(1000, new ActionListener() {
	                        		public void actionPerformed(ActionEvent e) {
	                        			// Do the task here
	                        			int battleTime = 210;
	                        			try {
											battleTime = Square5.this.match.getInt("Time");
	                        			...
	                        			if((timeSetInSeconds-1) == 0){// Endgame
	                        				...
	                        				try {
												BUS.sendEndGame(dos, Square5.this.user.getInt("player ID")+"",null);
	                        ...
	                        case "EndGame": {
	                        	...
	                        	if (obj.getString("Result").equals("Win")) {
	                        		if (obj.getString("player ID").equals(Square5.this.user.getString("player ID"))) {
	                        	...
	                        	btnUsername.setText(Square5.this.user.getString("Username"));
